...
struct KeyExchangeTests {
    ...
    @Test("Generate P-384 key pair")
    func generateP384() throws {
        let keyExchange = try KeyExchange.generate(for: .secp384r1)

        #expect(keyExchange.group == .secp384r1)
        #expect(keyExchange.publicKeyBytes.count == 97) // Uncompressed point
        #expect(keyExchange.publicKeyBytes[0] == 0x04) // Uncompressed point prefix
    }

    @Test("P-384 key agreement")
    func p384KeyAgreement() throws {
        let alice = try KeyExchange.generate(for: .secp384r1)
        let bob = try KeyExchange.generate(for: .secp384r1)

        let aliceShared = try alice.sharedSecret(with: bob.publicKeyBytes)
        let bobShared = try bob.sharedSecret(with: alice.publicKeyBytes)

        #expect(aliceShared.rawRepresentation == bobShared.rawRepresentation)
        #expect(aliceShared.rawRepresentation.count == 48) // P-384 shared secret is 48 bytes
    }

    @Test("P-384 KeyShareEntry")
    func p384KeyShareEntry() throws {
        let keyExchange = try KeyExchange.generate(for: .secp384r1)
        let entry = keyExchange.keyShareEntry()

        #expect(entry.group == .secp384r1)
        #expect(entry.keyExchange == keyExchange.publicKeyBytes)
    }
    ...
    func unsupportedGroup() throws {
        #expect(throws: KeyExchangeError.self) {
            _ = try KeyExchange.generate(for: .secp521r1)
        }
    }
...
struct SignatureTests {
    ...
    func certificateVerifyContent() throws {
        ...
        let serverContent = CertificateVerify.constructSignatureContent(
            transcriptHash: transcriptHash,
            isServer: true
        )

        let clientContent = CertificateVerify.constructSignatureContent(
            transcriptHash: transcriptHash,
            isServer: false
        )
