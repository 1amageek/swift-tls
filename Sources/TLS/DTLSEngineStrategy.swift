/// The portable ECDHE + certificate strategy for the swift-ssl DTLS engine.
///
...
/// front (no silent fallback). CA-path validation is not performed by this seam;
/// WebRTC authenticates the complete leaf through its negotiated fingerprint.
///
/// This is the only strategy used by the facade on Native, WASI, and Embedded.
import P2PCoreBytes
...
extension DTLSConfiguration {
    ...
    func makeDTLSEngineConfiguration() throws(TLSError) -> DTLSEngineConfiguration<TLSCryptoProvider> {
        ...
        var engine = DTLSEngineConfiguration<TLSCryptoProvider>(
            ...
            srtpProtectionProfiles: srtp?.wireProtectionProfiles,
            requireClientCertificate: requireClientCertificate,
            certificateChainDER: [leaf.der],
            signingScheme: .ecdsa_secp256r1_sha256
        )
        engine.prfContext = TLSCryptoProvider.dtlsPRFContext()
        engine.transcriptContext = TLSCryptoProvider.dtlsTranscriptContext()
        ...
        engine.recordProtectorFactory = { @Sendable (
            cipherSuite: DTLSCipherSuite,
            key: [UInt8],
            fixedIV: [UInt8]
        ) throws(DTLSRecordProtectionError) -> DTLSRecordProtectionContext in
            switch cipherSuite {
            case .ecdheEcdsaWithAes128GcmSha256,
                 .ecdheRsaWithAes128GcmSha256:
                return try TLSCryptoProvider.makeDTLSAES128RecordProtectionContext(
                    key: key,
                    fixedIV: fixedIV
                )
            case .ecdheEcdsaWithAes256GcmSha384:
                return try TLSCryptoProvider.makeDTLSAES256RecordProtectionContext(
                    key: key,
                    fixedIV: fixedIV
                )
            }
...
enum EmbeddedDTLSSeams {
    ...
    static func verifyPeerSignature(chain: [[UInt8]], signature: [UInt8], data: [UInt8]) throws(DTLSEngineError) -> Bool {
        ...
        let parsed = try parseCertificateEntry(peerLeaf)
    ...
    /// DTLS 1.2 Certificate entries in this facade may carry either the WebRTC
    /// X.509 leaf or an RFC-7250 raw public key. Each representation is parsed by
    /// its strict codec; accepting one is an explicit protocol capability rather
    /// than a permissive malformed-certificate fallback.
    private static func parseCertificateEntry(
        _ peerLeaf: [UInt8]
    ) throws(DTLSEngineError) -> SubjectPublicKeyInfoDER.Parsed {
        do {
            return try X509CertificateDER.subjectPublicKeyInfo(in: peerLeaf)
        } catch {
            do {
                return try SubjectPublicKeyInfoDER.parse(peerLeaf)
            } catch {
                throw .verificationFailed(
                    reason: "DTLS peer certificate is neither strict X.509 nor RFC-7250 SPKI"
                )
            }
        }
    }
