...
public struct ServerHello: Sendable {
    ...
    public init(
        ...
        guard random.count == TLSConstants.randomLength else {
            throw TLSWireError.decode(.invalidFormat("ServerHello random must be \(TLSConstants.randomLength) bytes, got \(random.count)"))
        }
        self.random = random
    ...
    public func encodeAsHandshakeBytes() throws(TLSWireError) -> [UInt8] {
        HandshakeCodec.encodeBytes(type: .serverHello, content: try encodeBytes())
    }
    ...
    public static func decode(from data: [UInt8]) throws(TLSWireError) -> ServerHello {
        ...
        guard let cipherSuite = CipherSuite(rawValue: cipherSuiteValue) else {
            throw TLSWireError.decode(.invalidFormat("Unknown cipher suite: \(cipherSuiteValue)"))
        }

        // legacy_compression_method
        let compressionMethod = try reader.wReadUInt8()
        guard compressionMethod == 0x00 else {
            throw TLSWireError.decode(.invalidFormat("Invalid compression method: \(compressionMethod)"))
        }

        // extensions — use appropriate context based on whether this is HRR
        let extensionData = try reader.wReadVector16()
        let isHRR = random == TLSConstants.helloRetryRequestRandom
