...
/// `Data`-based API by specialising the core at `C = TLSCryptoProvider` and
...
public struct TranscriptHash: Sendable {
    private var core: TLSCryptoCore.TLSTranscriptHash<TLSCryptoProvider>
    ...
    public init() {
        self.core = TLSCryptoCore.TLSTranscriptHash<TLSCryptoProvider>()
    }

    /// Initialize with specific cipher suite
    public init(cipherSuite: CipherSuite) {
        self.core = TLSCryptoCore.TLSTranscriptHash<TLSCryptoProvider>(cipherSuite: cipherSuite)
    }

    /// Internal init wrapping a core value (for copy / fromMessageHash)
    private init(core: TLSCryptoCore.TLSTranscriptHash<TLSCryptoProvider>) {
        self.core = core
    }
    ...
    var coreValue: TLSCryptoCore.TLSTranscriptHash<TLSCryptoProvider> { core }
    ...
    public static func fromMessageHash(
        ...
        let core = TLSCryptoCore.TLSTranscriptHash<TLSCryptoProvider>.fromMessageHash(
