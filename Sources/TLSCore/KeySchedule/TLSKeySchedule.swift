...
/// `C = TLSCryptoProvider` (swift-tls's own swift-crypto backend) and
...
public struct TLSKeySchedule: Sendable {
    ...
    private var core: TLSCryptoCore.TLSKeySchedule<TLSCryptoProvider>
    ...
    public init(cipherSuite: CipherSuite = .tls_aes_128_gcm_sha256) {
        self.core = TLSCryptoCore.TLSKeySchedule<TLSCryptoProvider>(cipherSuite: cipherSuite)
    }
    ...
    var coreValue: TLSCryptoCore.TLSKeySchedule<TLSCryptoProvider> {
        get { core }
        set { core = newValue }
    }
