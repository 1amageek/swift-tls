...
///
/// TLS uses fixed labels with empty context in many cases.
...
public struct TLSKeySchedule: Sendable {
    ...
    private func computeHash(_ data: Data) -> Data {
        switch cipherSuite {
        case .tls_aes_256_gcm_sha384:
            return Data(SHA384.hash(data: data))
        default:
    ...
    ///
    /// Uses the cipher-suite-appropriate hash function for all derivations.
    ///
    /// - Parameters:
    ///   - exporterMasterSecret: The exporter master secret derived during handshake
